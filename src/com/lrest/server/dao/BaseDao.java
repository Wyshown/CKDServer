...
public class BaseDao<T> {
     ...
     * @throws SQLException
     */
    public JsonObject findJonObjByPropery(String properyName, Object properyValue) throws Exception {
        ...
        try (Connection conn = DB.getConn()) {
            ...
            String sql = "select * from " + persistentClass.getAnnotation(Table.class).name()
                    + " where " + properyName + " =\"" + properyValue + "\" LiMIT 1";
            ps = conn.prepareStatement(sql);
            rs = ps.executeQuery();
            DBUtils.getDatas(jsonObject, rs);
        } catch (Exception e) {
            e.printStackTrace();
            throw e;
        } finally {
