...
public class LoginController extends BaseController {
    ...
    */
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @Consumes(MediaType.APPLICATION_JSON)
    public JsonObject editPassWord(String query) {
        try {
            this.jsonObject = new JsonParser().parse(query).getAsJsonObject();
            String userId = getJsonAsString(this.jsonObject, "userId");
            String oldpwd = getJsonAsString(this.jsonObject, "oldpwd");
            String loginPwd = getJsonAsString(this.jsonObject, "newpwd");

            Integer resultNo = this.userDao.doLogUp(userId,oldpwd,loginPwd);
            if (resultNo == 1) {
                return  error("原密码错误,不能修改密码");
            } else if (resultNo == 2) {
                return  success();
            } else {
                return error();
